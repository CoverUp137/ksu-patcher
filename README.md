...
脚本会自动跳过并继续处理其余分区，不会整体失败。日志里会显示
`未找到目标分区: xxx`，最终只在清单里列出实际提取到的分区。

**Q: 提示 `没有匹配到要修补的镜像`**
说明上一步没有提取到目标分区。请检查：
1. `partitions` 输入是否包含要修补的分区名（如 `boot,init_boot`）；
2. 查看「解包并提取分区」步骤打印的**分区清单**，确认固件里确实有该分区；
3. OTA 包（含 `payload.bin`）的分区名需与 payload 内一致。
